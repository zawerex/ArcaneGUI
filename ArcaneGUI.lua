local LuxtLib = {}
LuxtLib.__index = LuxtLib

function LuxtLib.new(libName, logoId)
    local self = setmetatable({}, LuxtLib)
    self:CreateWindow(libName, logoId)
    return self
end

function LuxtLib:CreateWindow(libName, logoId)
    local LuxtLibGui = Instance.new("ScreenGui")
    ...
    game:GetService("UserInputService").InputBegan:connect(function(current, ok) 
        if not ok then 
            if current.KeyCode.Name == oldKey then 
                if LuxtLibGui.Enabled == true then
                    LuxtLibGui.Enabled = false
                else
                    LuxtLibGui.Enabled = true
                end
            end
        end
    end)
    ...
    LuxtLibGui.Name = "LuxtLib"..libName
    LuxtLibGui.Parent = game.CoreGui
    LuxtLibGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ...
    self.LuxtLibGui = LuxtLibGui
    self.shadow = shadow
    self.MainFrame = MainFrame
    self.sideHeading = sideHeading
    self.tabFrame = tabFrame
    self.pageFolder = pageFolder
    self.framesAll = framesAll
end

function LuxtLib:Tab(tabText, tabId)
    ...
    tabBtn.MouseButton1Click:Connect(function()
        UpdateSize()
        for i,v in next, self.pageFolder:GetChildren() do
            UpdateSize()
            v.Visible = false
        end
        newPage.Visible = true
        for i,v in next, self.tabFrame:GetChildren() do
            if v:IsA("Frame") then
